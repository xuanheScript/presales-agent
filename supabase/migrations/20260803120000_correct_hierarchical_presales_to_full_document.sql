-- 将已部署的分层 Map/Reduce 售前执行契约修正为单次全文分析契约。
-- 兼容两种前置状态：旧 Map/Reduce 版本，或已是全文版本的新环境。

-- 旧工作单元仅是可重建的中间产物；历史最终估算版本保持不变。
DELETE FROM public.presales_execution_work_units;

ALTER TABLE public.presales_execution_work_units
  DROP CONSTRAINT IF EXISTS presales_execution_work_units_stage_check,
  DROP CONSTRAINT IF EXISTS presales_execution_work_units_status_check,
  DROP CONSTRAINT IF EXISTS presales_execution_work_units_level_check,
  DROP CONSTRAINT IF EXISTS presales_execution_work_units_execution_id_stage_unit_key_key;

ALTER TABLE public.presales_execution_work_units
  DROP COLUMN IF EXISTS level,
  DROP COLUMN IF EXISTS source_id,
  DROP COLUMN IF EXISTS parent_unit_key;

ALTER TABLE public.presales_execution_work_units
  ADD CONSTRAINT presales_execution_work_units_stage_check
    CHECK (stage = 'full_document_discovery'),
  ADD CONSTRAINT presales_execution_work_units_unit_key_check
    CHECK (unit_key = 'global'),
  ADD CONSTRAINT presales_execution_work_units_status_check CHECK (
    status IN ('pending', 'leased', 'retry_wait', 'succeeded', 'failed', 'cancelled')
  ),
  ADD CONSTRAINT presales_execution_work_units_execution_id_stage_unit_key_key
    UNIQUE (execution_id, stage, unit_key);

ALTER TABLE public.agent_execution_model_calls
  DROP CONSTRAINT IF EXISTS agent_execution_model_calls_stage_check;
ALTER TABLE public.agent_execution_model_calls
  ADD CONSTRAINT agent_execution_model_calls_stage_check
    CHECK (stage = 'full_document_discovery');
...
UPDATE public.agent_executions
 ...
 WHERE current_stage NOT IN (
   'planning', 'discovering', 'enriching', 'calculating', 'committing', 'complete'
 );
ALTER TABLE public.agent_executions
...
DROP FUNCTION IF EXISTS public.initialize_presales_execution_plan(
  UUID, UUID, JSONB, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB
);
DROP FUNCTION IF EXISTS public.save_presales_function_candidates(
  UUID, UUID, BIGINT, UUID, UUID, BIGINT, TEXT, UUID, UUID, TEXT,
  JSONB, JSONB, TEXT, BIGINT, BIGINT, INTEGER, TEXT
);
DROP FUNCTION IF EXISTS public.save_presales_reduction_artifact(
  UUID, UUID, BIGINT, TEXT, UUID, TEXT, TEXT, JSONB, TEXT, JSONB, TEXT
);
DROP FUNCTION IF EXISTS public.supersede_presales_map_work_unit(
  UUID, UUID, UUID, BIGINT, UUID, UUID, BIGINT, TEXT, JSONB
);

DROP TABLE IF EXISTS public.estimate_version_function_evidence;
DROP TABLE IF EXISTS public.presales_function_candidates;

DROP POLICY IF EXISTS "Owners can view presales execution work units"
  ON public.presales_execution_work_units;
DROP POLICY IF EXISTS "Owners can view presales execution model calls"
  ON public.agent_execution_model_calls;

-- 下方重新应用同批次的全文目标契约，用 CREATE OR REPLACE 替换旧 RPC，
-- 并重新声明租约、账本、原子提交、RLS 与授权。
...
DROP FUNCTION IF EXISTS public.commit_presales_execution(
...
DROP FUNCTION IF EXISTS public.finish_presales_execution(
